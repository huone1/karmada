...
func (g *genericScheduler) Schedule(ctx context.Context, placement *policyv1alpha1.Placement, spec *workv1alpha2.ResourceBindingSpec) (result ScheduleResult, err error) {
	...
	clusters := g.selectClusters(clustersScore, placement, spec)
...
func (g *genericScheduler) findClustersThatFit(
	...
	for _, c := range clusters {
		if result := fwk.RunFilterPlugins(ctx, placement, spec, c.Cluster()); !result.IsSuccess() {
...
func (g *genericScheduler) selectClusters(clustersScore framework.ClusterScoreList, placement *policyv1alpha1.Placement, spec *workv1alpha2.ResourceBindingSpec) []*clusterv1alpha1.Cluster {
